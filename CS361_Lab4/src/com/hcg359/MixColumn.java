...
/**
 * Created by charlesgong on 6/29/17.
 */
...
public class MixColumn {
    ...
    public int[][] mixColumn2 (int[][] st, int c) {
        ...
        st[c][0] = mul(2,a[0]) ^ a[2] ^ a[3] ^ mul(3,a[1]);
        st[c][1] = mul(2,a[1]) ^ a[3] ^ a[0] ^ mul(3,a[2]);
        st[c][2] = mul(2,a[2]) ^ a[0] ^ a[1] ^ mul(3,a[3]);
        st[c][3] = mul(2,a[3]) ^ a[1] ^ a[2] ^ mul(3,a[0]);

        return st;
    } // mixColumn2

    public int[][] invMixColumn2 (int[][] st, int c) {
        int[] a = new int[4];
        ...
        st[c][0] = mul(0xE,a[0]) ^ mul(0xB,a[1]) ^ mul(0xD, a[2]) ^ mul(0x9,a[3]);
        st[c][1] = mul(0xE,a[1]) ^ mul(0xB,a[2]) ^ mul(0xD, a[3]) ^ mul(0x9,a[0]);
        st[c][2] = mul(0xE,a[2]) ^ mul(0xB,a[3]) ^ mul(0xD, a[0]) ^ mul(0x9,a[1]);
        st[c][3] = mul(0xE,a[3]) ^ mul(0xB,a[0]) ^ mul(0xD, a[1]) ^ mul(0x9,a[2]);
